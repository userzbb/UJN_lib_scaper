...
def worker(username, password, day_prefix):
    ...
    # Infinite retry loop for transient errors (Captcha/Network)
    # We MUST NOT skip passwords due to network glitches or bad OCR
    error_streak = 0
    while True:
        if stop_event.is_set():
            return
        ...
        if result == "SUCCESS":
            ...
            # Network error, sleep with backoff
            error_streak += 1
            sleep_time = min(0.5 * error_streak, 5.0)  # Cap at 5 seconds
